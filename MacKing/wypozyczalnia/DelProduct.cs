...
namespace wypozyczalnia
{
    public partial class DelProduct : UserControl
    {
        ...
        void pullCategory()
        {
            string connection = "Data Source=Damian-PC\\SQLEXPRESS;Initial Catalog=Fastfood;Integrated Security=True";
        ...
        void delProduct(object sender, EventArgs e)
        {
            ...
            string query = $"SELECT  DISTINCT ZamowienieID FROM SzczegolyZamowienia WHERE ProduktID={id}";
            SqlCommand sqlCommand = new SqlCommand(query, conn);
            SqlDataReader dr = sqlCommand.ExecuteReader();
            ...
            foreach (int a in zamowienia)
            {
                ...
                if (dr2.HasRows)
                {
                    ...
                    if (i == 1)
                    {
                        query3 = $"DELETE FROM SzczegolyZamowienia WHERE ZamowienieID={a}";
                        sqlCommand3 = new SqlCommand(query3, conn);
                        sqlCommand3.ExecuteScalar();
                        query3 = $"DELETE FROM Zamowienia WHERE ID={a}";
                        sqlCommand3 = new SqlCommand(query3, conn);
                        sqlCommand3.ExecuteScalar();
                    }
                    else
                    {
                        query3 = $"DELETE FROM SzczegolyZamowienia WHERE ProduktID={id} AND ZamowienieID={a}";
                        sqlCommand3 = new SqlCommand(query3, conn);
                        sqlCommand3.ExecuteScalar();
                    }
                } 
